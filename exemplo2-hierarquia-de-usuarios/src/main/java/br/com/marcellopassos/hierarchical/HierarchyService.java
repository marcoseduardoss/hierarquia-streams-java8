...
public class HierarchyService<T extends IElement<R>, R> implements IHierarchyService<T, R> {
    ...
    public Collection<T> getDescendants(T element) {
        ...
        return children.isEmpty() ? Collections.emptyList() : Stream.concat(children.stream(), children.stream()
    ...
    public T getParent(T element) {
        return Optional.ofNullable(element.getParentPk())
                .map(this::findById)
                .orElse(null);
    }
    ...
    private boolean checkIsDescendant(T ancestral, T parent) {
        return parent.checkId(ancestral.getPk()) || this.isDescendant(parent, ancestral);
    }
    ...
    public boolean isRoot(T element) {
        return !Optional.ofNullable(element.getParentPk()).isPresent();
    }
